...
<div class="row">
    <div class="col-xs-12">
        <p class="payment_module">
            <a class="s2ppaymentmethod" href="{$s2p_module_obj->get_payment_link( ['method_id' => {$method_arr.method.method_id|escape:'html'}] )}" {if $moduleSettings["{$settings_prefix|escape:'html'}LOADING_MODAL"]} onclick="s2p_display_loading_layer()" {/if} title="{l s='Pay by' mod='smart2pay'} {$method_arr.method.display_name|escape:'html'}">
                <img src="{$method_arr.method.logo_url|escape:'html'}" alt="{l s='Pay by' mod='smart2pay'} {$method_arr.method.display_name|escape:'html'}" class="s2ppaymentlogo" />
                ...
                {if $method_arr.settings.surcharge_percent != 0 || $method_arr.settings.surcharge_amount != 0}
                    ...
                    {else}
                        ...
                        {if $method_arr.settings.surcharge_percent != 0}{($method_arr.settings.surcharge_percent+1-1)|escape:'html'}%{/if}
                        {if $method_arr.settings.surcharge_percent != 0 && $method_arr.settings.surcharge_amount > 0} + {/if}
                        {if $method_arr.settings.surcharge_amount != 0}
                            {if $config_opt_currency == $display_options.from_front}
                                {displayPrice price=$method_arr.settings.surcharge_amount_converted currency=$methods_detected_currency}
                            {else}
                                {displayPrice price=$method_arr.settings.surcharge_amount currency=$method_arr.settings.surcharge_currency_id}
                            {/if}
                        )
                        {/if}
                    {/if}
        ...
        {else}
<p class="payment_module">
    <a href="{$s2p_module_obj->get_payment_link( ['method_id' => {$method_arr.method.method_id|escape:'html'}] )}"
        {if $moduleSettings["{$settings_prefix|escape:'html'}LOADING_MODAL"]} onclick="s2p_display_loading_layer()" {/if}
       title="{l s='Pay by' mod='smart2pay'} {$method_arr.method.display_name|escape:'html'}"
        {if $smarty.const._PS_VERSION_ < 1.5}
            style="min-height: 50px"
        {/if}>
        <span style="width: 86px; height: 49px;"><img src="{$method_arr.method.logo_url|escape:'html'}" alt="{l s='Pay by' mod='smart2pay'} {$method_arr.method.display_name|escape:'html'}" style="max-width: 86px; max-height: 49px;" /></span>
        {l s='Pay by' mod='smart2pay'} {$method_arr.method.display_name|escape:'html'}
        {if $method_arr.settings.surcharge_percent != 0 || $method_arr.settings.surcharge_amount != 0}
