        ...
        <div class="col-xs-12">
            <p class="payment_module">
                <a class="s2ppaymentmethod" href="{$redirect_URL}" title="{l s="Pay by {$method_name} via Smart2Pay" mod="{$module_name}"}">
                    <img src="{$this_path_bw}logo.png" alt="{l s="Pay by {$method_name} via Smart2Pay" mod="{$module_name}"}" class="s2ppaymentlogo" style="top: 35px;" />
                    IN S2P!!! {l s="Pay by {$method_name}" mod="{$module_name}"} ({$surcharge_percent} + {$surcharge_amount})
