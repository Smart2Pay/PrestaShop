        ...
        try {
            ...
            if (($result = $reader->get($ip))) {
                ...
                and ($s2p_module = Module::getInstanceByName(self::S2P_MODULE_NAME))) {
                    $s2p_module->writeLog('Detected country [' . $result['country']['iso_code'] . '] for IP [' . $ip . ']', ['type' => 'detection']);
                }
            }
        } catch (Exception $e) {
    ...
    private function process_post_data()
    {
        ...
        if (Tools::isSubmit('submit_test_detection')) {
            ...
            if (empty($post_data['errors_buffer'])) {
                ...
                $detection_arr = [];
                $detection_arr['country'] = ['name' => 'N/A', 'code' => ''];
                $detection_arr['continent'] = ['name' => 'N/A', 'code' => ''];
