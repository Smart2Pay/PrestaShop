...
class Smart2pay extends PaymentModule
{
    ...
    /** @var bool|Smart2Pay_SDK_Interface */
    ...
    public function S2P_add_css($file, $file_name = '')
    {
        ...
        if (version_compare(_PS_VERSION_, '1.5', '<')) {
            ...
            $this->context->controller->registerStylesheet('modules-' . $this->name . '_' . $file_name, 'modules/' . $this->name . '/views/css/' . $file_name, ['media' => 'all', 'priority' => 150]);
        }
    }
    ...
    public function prepare_send_form()
    {
        ...
        if (empty(self::$s2p_sdk_obj)) {
            ...
            $this->redirect_to_step1(['errors' => [$this->l('Couldn\'t initialize Smart2Pay SDK.')]]);
        ...
        if (!($moduleSettings = $this->getSettings())) {
            ...
            $this->redirect_to_step1(['errors' => [$this->l('Couldn\'t obtain Smart2Pay plugin settings.')]]);
        ...
        if (!($api_credentials = $sdk_obj->get_api_credentials($moduleSettings))) {
            ...
            $this->redirect_to_step1(['errors' => [$this->l('Couldn\'t obtain Smart2Pay API credentials.')]]);
        ...
        if (empty($cart)
         or !($cart_products = $cart->getProducts())) {
            ...
            $this->redirect_to_step1(['errors' => [$this->l('Couldn\'t get cart from context.')]]);
        ...
        if (!Validate::isLoadedObject($customer)) {
            $this->redirect_to_step1(['errors' => [$this->l('Couldn\'t load customer data.')]]);
        }
        ...
        if (empty($method_id)
         or !($payment_method = $this->method_details_if_available($method_id, null, $moduleSettings['environment']))) {
            $this->writeLog('Payment method #' . $method_id . ', environment ' . $moduleSettings['environment'] . ' could not be loaded, or it is not available', ['type' => 'error']);

            $this->redirect_to_step1(['errors' => [$this->l('Payment method could not be loaded or it is not available.')]]);
        }
        ...
        if (empty($payment_method['method_settings']['surcharge_currency'])
         ...
         or !($surcharge_currency_obj = new Currency($surcharge_currency_id))) {
            $this->writeLog('Payment method #' . $method_id . ' (' . $payment_method['method_details']['display_name'] . '), ' .
                             ...
                             ' [' . (!empty($payment_method['method_settings']['surcharge_currency']) ? $payment_method['method_settings']['surcharge_currency'] : '???') . '].', ['type' => 'error']);

            $this->redirect_to_step1(['errors' => [$this->l('Payment method has an invalid currency code.')]]);
        ...
        if (!($new_transaction = $this->save_transaction($transaction_arr))) {
            $this->writeLog('Failed creating transaction for order [' . $orderID . '].', ['type' => 'error', 'order_id' => $orderID]);

            $this->redirect_to_step1(['errors' => [$this->l('Failed creating transaction for order. Please try again.')]]);
        ...
        if ($method_id == self::PAYM_SMARTCARDS) {
            if (!($payment_request = $sdk_obj->card_init_payment($payment_arr, $moduleSettings))) {
                ...
                $this->redirect_to_step1(['errors' => [$error_msg]]);
        ...
        if (!($new_transaction = $this->save_transaction($transaction_arr))) {
            $this->writeLog('Failed saving transaction with payment id [' . $transaction_arr['payment_id'] . '] for order [' . $orderID . '].', ['type' => 'error', 'order_id' => $orderID]);

            $this->redirect_to_step1(['errors' => [$this->l('Failed saving transaction for order. Please try again.')]]);
        ...
        $this->writeLog('Redirecting to payment page for payment id [' . $transaction_arr['payment_id'] . '], order [' . $orderID . '].', ['order_id' => $orderID]);
    ...
    public function prepare_notification()
    {
        ...
        switch ($notification_type) {
            case $notification_obj::TYPE_PAYMENT:
                ...
                $this->writeLog('Received ' . $status_title . ' notification for transaction ' . $payment_arr['merchanttransactionid'] . '.', ['order_id' => $payment_arr['merchanttransactionid']]);
                ...
                switch ($payment_arr['status']['id']) {
                    ...
                    case S2P_SDK\S2P_SDK_Meth_Payments::STATUS_OPEN:
                        ...
                        and in_array($transaction_arr['method_id'], [self::PAYM_BANK_TRANSFER, self::PAYM_MULTIBANCO_SIBS])
                        ...
                        and ($extra_vars_arr = Smart2Pay_Helper::parse_string($transaction_arr['extra_data']))) {
                            ...
                            if (!empty($template)) {
                                $this->writeLog('Sending payment instructions email for order [' . (version_compare(_PS_VERSION_, '1.5', '<') ? $order->id : $order->reference) . ']', ['order_id' => $order->id]);
                    ...
                    case S2P_SDK\S2P_SDK_Meth_Payments::STATUS_SUCCESS:
                        ...
                        if (strcmp($orderAmount_check, $payment_arr['amount']) != 0
                         or $orderCurrency != $payment_arr['currency']) {
                            $this->writeLog('Smart2Pay :: notification has different amount[' . $orderAmount_check . '/' . $payment_arr['amount'] . '] ' .
                                                   ' and/or currency [' . $orderCurrency . '/' . $payment_arr['currency'] . ']. Please contact support@smart2pay.com.', ['type' => 'error', 'order_id' => $order->id]);
                            ...
                            $this->writeLog('Smart2Pay :: Couldn\'t get method details [' . $payment_arr['methodid'] . ']', ['type' => 'error', 'order_id' => $order->id]);
                        }
                        ...
                        elseif ($this->get_quick_last_order_status($order) == $plugin_settings[self::CONFIG_PREFIX . 'ORDER_STATUS_ON_SUCCESS']) {
                            $this->writeLog('Order already on success status.', ['type' => 'error', 'order_id' => $order->id]);
                        } else {
                            $orderAmount = number_format($orderAmount_check / 100, 2, '.', '');

                            $this->writeLog('Order [' . (version_compare(_PS_VERSION_, '1.5', '<') ? $order->id : $order->reference) . '] has been paid', ['order_id' => $order->id]);
                            ...
                            if (!empty($plugin_settings[self::CONFIG_PREFIX . 'CREATE_INVOICE_ON_SUCCESS'])) {
                                $this->check_order_invoices($order, ['check_delivery' => (!empty($plugin_settings[self::CONFIG_PREFIX . 'AUTOMATE_SHIPPING']) ? true : false)]);
                            }
                        }
        ...
        if ($notification_obj->respond_ok()) {
            $this->writeLog('--- Sent OK -------------------------------', ['type' => 'info', 'order_id' => ($order ? $order->id : 0)]);
        } else {
            if ($notification_obj->has_error()
            and ($error_arr = $notification_obj->get_error())) {
                $error_msg = 'Error: ' . $error_arr['display_error'];
    ...
    private function process_post_data()
    {
        ...
        elseif (Tools::isSubmit('submit_main_data') and isset($_POST[self::CONFIG_PREFIX . 'ENABLED'])) {
            ...
            foreach ($this->getConfigFormInputs() as $input) {
                ...
                and in_array($input['name'], [self::CONFIG_PREFIX . 'SITE_ID_LIVE', self::CONFIG_PREFIX . 'APIKEY_LIVE'])) {
                    $skipValidation = true;
                }

                if (in_array($formValues[self::CONFIG_PREFIX . 'ENV'], ['demo', 'live'])
                and in_array($input['name'], [self::CONFIG_PREFIX . 'SITE_ID_TEST', self::CONFIG_PREFIX . 'APIKEY_TEST'])) {
                    $skipValidation = true;
                }
    ...
    public function hookDisplayOrderDetail($params)
    {
        ...
        if (function_exists('smartyRegisterFunction')) {
            smartyRegisterFunction($this->context->smarty, 'function', 'S2P_displayPrice', ['Tools', 'displayPriceSmarty']);
        }
    ...
    public function hookPaymentOptions($params)
    {
        ...
        foreach ($template_data['payment_methods'] as $method_id => $method_arr) {
            ...
            $payment_option
                ...
                ->setAction($this->get_payment_link(['method_id' => $method_arr['method']['method_id']]))
    ...
    public function detect_country($ip = false)
    {
        ...
        if (!empty(self::$cache['detected_country_ip']) and self::$cache['detected_country_ip'] == $ip) {
            $this->writeLog('Cached country for IP [' . $ip . '] is [' . self::$cache['detected_country'] . ']', ['type' => 'detection']);
        ...
        if (!($smart2pay_detection = Module::getInstanceByName(self::S2P_DETECTOR_NAME))) {
            $this->writeLog('Couldn\'t obtain Smart2Pay Detection instance. Make sure plugin is installed correctly.', ['type' => 'detection']);
    ...
    public function changeOrderStatus($order, $statusId)
    {
        ...
        if (!Validate::isLoadedObject($order)) {
            $this->writeLog('Can not apply order state #' . $statusId . ' to order - Order cannot be loaded', ['type' => 'error']);

            return false;
        }

        if (!Validate::isLoadedObject($orderState)) {
            $this->writeLog('Can not apply order state #' . $statusId . ' to order #' . $order->id . ' - Order state cannot be loaded', ['type' => 'error', 'order_id' => $order->id]);
    ...
    public function get_smart2pay_countries()
    {
        ...
        foreach ($country_rows as $country_arr) {
            ...
            self::$cache['all_id_countries'][$country_arr['country_id']] = ['code' => $country_arr['code'], 'name' => $country_arr['name']];
    ...
    protected function get_country_iso()
    {
        ...
        if (($forced_country = $this->force_country())) {
            ...
            $this->writeLog('Using country [' . $country_iso . '] from programming restriction.', ['type' => 'detection']);
    ...
    public function get_methods_for_country($country_iso = null, $currency_obj = null, $params = false)
    {
        ...
        if (empty($params['opt_currency']) or !in_array($params['opt_currency'], [self::OPT_FEE_CURRENCY_FRONT, self::OPT_FEE_CURRENCY_ADMIN])) {
            $params['opt_currency'] = self::OPT_FEE_CURRENCY_FRONT;
        }
        if (empty($params['opt_amount']) or !in_array($params['opt_amount'], [self::OPT_FEE_AMOUNT_SEPARATED, self::OPT_FEE_AMOUNT_TOTAL_FEE, self::OPT_FEE_AMOUNT_TOTAL_ORDER])) {
        ...
        $this->writeLog('Found [' . count($country_methods_arr['methods']) . '] methods for country [' . $country_iso . ']', ['type' => 'detection']);
    ...
    public function method_details_if_available($method_id, $country_iso = null, $environment = false)
    {
        ...
        $this->writeLog('Using method ID [' . $method_id . '] for country [' . $country_iso . '], environment [' . $environment . '].', ['type' => 'detection']);
        ...
        */
        ...
        return [
            ...
            [
                ...
                'required' => true,
                'options' => [
                    'query' => $this->getConfigFormSelectInputOptions('country_list', ['no_option_title' => $this->l('- Don\'t force country -')]),
                    'id' => 'id',
                    'name' => 'name',
                ],
            ...
            [
                ...
                'required' => true,
                'options' => [
                    'query' => $this->getConfigFormSelectInputOptions('country_list', ['no_option_title' => $this->l('- Country From Billing Address -')]),
                    'id' => 'id',
                    'name' => 'name',
                ],
                '_transform' => ['trim', 'toupper'],
