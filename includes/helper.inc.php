...
**/
if (!class_exists('Smart2PayHelper', false)) {
    class Smart2PayHelper
    {
         ...
         */
        public static function getTotalShippingCost($cart_obj)
        {
        ...
        public static function cartProductsToString($products_arr, $cart_original_amount, $params = false)
        {
            ...
            if (empty($params['payment_method'])
                or !in_array(
                    ...
                    [
                        \S2P_SDK\S2P_SDK_Meth_Methods::METHOD_KLARNA_INVOICE,
                        \S2P_SDK\S2P_SDK_Meth_Methods::METHOD_KLARNA_PAYMENTS,
                    ]
                )
            ) {
            ...
            // 1 - product, 2 - shipping, 3 - handling, 4 - discount, 5 - physical, 6 - shipping fee, 7 - sales tax,
            // 9 - digital
            ...
            $amount_to_pay = (float) $params['amount_to_pay'];
            ...
            if ($params['total_surcharge'] != 0) {
                ...
                foreach ($articles_arr as $knti => $article_arr) {
                    ...
                    $total_article_surcharge = (
                        ($articles_meta_arr[$knti]['price_perc'] * $params['total_surcharge']) / 100
                    );

                    $article_unit_surcharge = Tools::ps_round(
                ...
                // If after applying all surcharge amounts
                //  as percentage of each product price we still have a difference,
            ...
            $return_arr['buffer'] = Tools::substr($return_arr['buffer'], 0, -1);
        ...
        public static function getDocumentationFileName()
        {
        ...
        public static function getDocumentationUrl()
        {
            return Tools::getShopDomainSsl(true, true)
                . __PS_BASE_URI__ . 'modules/smart2pay/' . self::getDocumentationFileName();
        }

        public static function getDocumentationPath()
        {
            return _PS_MODULE_DIR_ . 'smart2pay/' . self::getDocumentationFileName();
        }

        public static function getReturnUrl($module_name)
        {
            if (version_compare(_PS_VERSION_, '1.5', '>=')) {
                return
                    Tools::getShopDomainSsl(true, true)
                    . __PS_BASE_URI__ . 'index.php?fc=module&module='
                    . $module_name . '&controller=returnHandler';
            }

            return Tools::getShopDomainSsl(true, true)
                . __PS_BASE_URI__ . 'modules/' . $module_name . '/pre15/returnhandler.php';
        }

        public static function convertPrice($amount, $currency_from = null, $currency_to = null)
        {
            if (version_compare(_PS_VERSION_, '1.5', '>=')
        ...
        public static function generateAncientForm($fields, $form_data, $form_values)
        {
            ...
            $form_buffer = '<form action="' . Tools::safeOutput($_SERVER['REQUEST_URI']) . '" method="post">' .
                '<fieldset>' . (!empty($fields[0]['form']['legend']['title'])
                    ? ' <legend>' . $fields[0]['form']['legend']['title'] . '</legend>'
                    : '');
            ...
            if (!empty($fields[0]['form']['input']) and is_array($fields[0]['form']['input'])) {
                foreach ($fields[0]['form']['input'] as $input_arr) {
                    ...
                    $form_buffer .= '<label '
                        . (
                            (!empty($input_arr['hint']) and is_array($input_arr['hint']))
                            ? 'title="' . implode('', $input_arr['hint']) . '"'
                            : ''
                        )
                        . '>' . $input_arr['label']
                        . (
                            !empty($input_arr['required'])
                            ? '<span style="color:red">*</span>'
                            : ''
                        )
                        . '</label><div class="margin-form">';
                    ...
                    switch ($input_arr['type']) {
                        ...
                        case 'text':
                            ...
                            $form_buffer .= '<input type="text"' .
                                ' name="' . $input_arr['name'] . '"' .
                                ' value="' . (!is_null($current_value) ? $current_value : '') . '"' .
                                ' style="width: 550px;" />';
                            break;

                        case 'select':
                            $form_buffer .= '<select name="' . $input_arr['name'] . '">';
                            if (!empty($input_arr['options'])
                                and is_array($input_arr['options'])
                                and !empty($input_arr['options']['query'])
                                and is_array($input_arr['options']['query'])
                            ) {
                                ...
                                foreach ($input_arr['options']['query'] as $query_arr) {
                                    if (!isset($query_arr[$input_arr['options']['id']])
                                        or !isset($query_arr[$input_arr['options']['name']])
                                    ) {
                                        continue;
                                    }
            ...
            $form_buffer .= '<div style="text-align: center;">' .
                '<input type="submit"' .
                'name="' . $form_data['submit_action'] . '"' .
                'value="' . (!empty($fields[0]['form']['submit']['title'])
                    ? $fields[0]['form']['submit']['title']
                    : 'Save') . '"' .
                'class="' . (!empty($fields[0]['form']['submit']['class'])
                    ? $fields[0]['form']['submit']['class']
                    : 'button')
                . '" /></div>' .
                '</fieldset></form>';
        ...
        public static function stringToValue($str)
        {
            ...
            $tch = Tools::substr($str, 0, 1);
            if ($tch == '\'' or $tch = '"') {
                $str = Tools::substr($str, 1);
            }

            $tch = Tools::substr($str, -1);
            if ($tch == '\'' or $tch = '"') {
        ...
        public static function toString($lines_data)
        {
            ...
            foreach ($lines_data as $key => $val) {
                ...
                // In normal cases there cannot be '=' char in key
                // so we interpret that value should just be passed as-it-is
        ...
        public static function quickInsert($into, $arr, $params = false)
        {
                ...
                if (is_array($val)) {
                    ...
                    if (empty($val['raw_field'])) {
                        if (!empty($params['secure'])) {
                            $field_value = self::prepareData($field_value);
                        }

                        $field_value = '\'' . $field_value . '\'';
                    }
                } else {
                    $field_value = '\'' . (!empty($params['secure']) ? self::prepareData($val) : $val) . '\'';
                }
            ...
            return 'INSERT INTO `' . $into . '` SET ' . Tools::substr($return, 0, -2);
        }

        public static function quickEdit($into, $arr, $params = false)
        {
            if (!is_array($arr) or !count($arr)) {
                return '';
            }
            ...
            return 'UPDATE `' . $into . '` SET ' . Tools::substr($return, 0, -2);
        }

        public static function getMainNotificationRequestParams()
        {
            return [
                'MethodID' => 0,
                'NotificationType' => '',
        ...
        public static function normalizeNotificationRequest($request_arr)
        {
            $default_main_params = self::getMainNotificationRequestParams();
        ...
        {
            parse_str(self::getPhpRawInput(), $response);
         ...
         * @return string
         */
        public static function getPhpRawInput()
        {
            static $input;
        ...
        public static function recomposeHashString($raw_input = null)
        {
            if ($raw_input === null) {
                $raw_input = self::getPhpRawInput();
            }
        ...
        public static function validateDbDate($str)
        {
            return date(self::SQL_DATE, self::parseDbDate($str));
        }

        public static function validateDbDatetime($str)
        {
            return date(self::SQL_DATETIME, self::parseDbDate($str));
        }
